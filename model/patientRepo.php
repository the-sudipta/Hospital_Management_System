...
//            throw new Exception("Query failed: " . $conn->error);
            $_SESSION['error_location'] = "Database -> patientRepo -> findAllPatients()";
...
//            throw new Exception("No rows found in the 'user' table.");
            $_SESSION['error_location'] = "Database -> patientRepo -> findAllPatients()";
            $_SESSION['database_error'] = "No rows found in the 'patient' table.";
            global $routes;
            $database_error_page = $routes["database_error"];
            header("Location: {$database_error_page}");
        }
...
//            throw new Exception("Prepare statement failed: " . $conn->error);
            $_SESSION['error_location'] = "Database -> patientRepo -> findPatientByID()";
...
function updatePatient($name, $dob, $gender, $contact, $address, $id)
{
    ...
    try {
        ...
        if (!$stmt) {
//            throw new Exception("Prepare statement failed: " . $conn->error);
            $_SESSION['error_location'] = "Database -> patientRepo -> updatePatient()";
            $_SESSION['database_error'] = "Prepare statement failed: " . $conn->error;
            global $routes;
            $database_error_page = $routes["database_error"];
            header("Location: {$database_error_page}");
...
//        echo "Error: " . $e->getMessage();
        $_SESSION['error_location'] = "Database -> patientRepo -> updateUser()";
        $_SESSION['database_error'] = $e->getMessage();
        global $routes;
        $database_error_page = $routes["database_error"];
        header("Location: {$database_error_page}");
...
function deletePatient($id) {
    ...
    // Construct the SQL query
    $updateQuery = "DELETE FROM `patient`
                    WHERE id = ?";
    ...
    try {
        ...
        if (!$stmt) {
//            throw new Exception("Prepare statement failed: " . $conn->error);
            $_SESSION['error_location'] = "Database -> patientRepo -> deletePatient()";
            $_SESSION['database_error'] = "Prepare statement failed: " . $conn->error;
            global $routes;
            $database_error_page = $routes["database_error"];
            header("Location: {$database_error_page}");
...
function createPatient($name, $dob, $gender, $contact, $address) {
    ...
    // Construct the SQL query
    $insertQuery = "INSERT INTO `patient` (name, dob, gender, contact, address) VALUES (?, ?, ?, ?, ?)";
    ...
    try {
        ...
        // Handle the exception, you might want to log it or return false
//        echo "Error: " . $e->getMessage();
        $_SESSION['error_location'] = "Database -> patientRepo -> createPatient()";
        $_SESSION['database_error'] = $e->getMessage();
        global $routes;
        $database_error_page = $routes["database_error"];
        header("Location: {$database_error_page}");
